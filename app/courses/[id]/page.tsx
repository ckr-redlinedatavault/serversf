...
export default function CourseDetailPage() {
    ...
    if (loading) return (
        <div className="h-screen bg-[#050505] flex flex-col items-center justify-center gap-4 text-zinc-700">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest animate-pulse">Accessing Secure Records...</p>
        </div>
    );
    ...
    if (!course) return (
        <div className="h-screen bg-[#050505] flex flex-col items-center justify-center gap-6 text-zinc-600">
            ...
            <div className="text-center">
                <p className="font-black text-xl text-white mb-2 uppercase tracking-tighter">Authorization Failed</p>
                <p className="text-sm font-medium">This module address does not exist in our main records.</p>
                <Link href="/courses" className="mt-8 inline-block text-[10px] font-black text-[#92E3A9] underline uppercase tracking-widest">Back to Directory</Link>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#92E3A9] selection:text-black">
            ...
            <nav className="w-full bg-[#92E3A9] px-6 py-4 md:px-24 flex items-center justify-between sticky top-0 z-50 shadow-2xl">
                <Link href="/courses" className="flex items-center gap-3 group">
                    ...
                    <span className="text-[10px] font-black tracking-[0.2em] text-zinc-900 uppercase">Back to Modules</span>
                </Link>
                <div className="flex gap-4 md:gap-8 items-center">
                    <div className="hidden sm:flex items-center gap-6 mr-6">
                        <span className="text-[10px] font-black text-zinc-900/40 uppercase">Instructor: <span className="text-zinc-900">{course.instructorName}</span></span>
                    </div>
                    <button className="bg-zinc-900 text-[#92E3A9] px-6 py-2.5 rounded-xl font-black text-[10px] hover:bg-black transition-all shadow-xl shadow-black/10 uppercase tracking-widest">
                        Enroll Now
                    </button>
                </div>
            </nav>
                ...
                <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                    <div>
                        <div className="flex flex-wrap items-center gap-4 mb-8">
                            <div className="px-3 py-1 bg-[#92E3A9]/10 border border-[#92E3A9]/20 rounded-full text-[10px] font-black text-[#92E3A9] uppercase tracking-widest">
                        ...
                        <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-8 uppercase text-balance">
                            {course.title}
                        </h1>
                        ...
                        <div className="flex flex-wrap items-center gap-10">
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black text-[#92E3A9] uppercase mb-1 tracking-widest opacity-60">Status</span>
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.4)]" />
                                    <span className="text-sm font-black uppercase text-white tracking-widest">Live Mission</span>
                                </div>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black text-[#92E3A9] uppercase mb-1 tracking-widest opacity-60">Rating</span>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black text-[#92E3A9] uppercase mb-1 tracking-widest opacity-60">Enrolled</span>
                                <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-20 py-16 md:py-24">
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                                <div className="p-10 bg-zinc-900/20 border border-zinc-900 rounded-[2.5rem] hover:border-[#92E3A9]/30 transition-all">
                                    <h4 className="text-[#92E3A9] font-black text-[10px] uppercase tracking-[0.3em] mb-6">What you will learn</h4>
                                ...
                                <div className="p-10 bg-zinc-900/20 border border-zinc-900 rounded-[2.5rem] hover:border-[#92E3A9]/30 transition-all">
                                    <h4 className="text-[#92E3A9] font-black text-[10px] uppercase tracking-[0.3em] mb-6">Target Audience</h4>
                                    <div className="flex flex-wrap gap-3">
                                        {course.targetAudience?.map((aud: string, idx: number) => (
                                            <div key={idx} className="px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-xl text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                                                {aud}
                                            </div>
                                        ))}
                                    </div>
                                        ...
                                        >
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] font-black text-zinc-700 uppercase tracking-widest mt-1">{module.lessons?.length} Phases Assigned</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-8 pt-0 border-t border-zinc-900 space-y-2">
                                                {module.lessons?.map((lesson: any, lIdx: number) => (
                                                    <div key={lIdx} className="flex items-center justify-between p-4 bg-zinc-900/20 border border-white/0 hover:border-white/5 rounded-2xl hover:bg-zinc-900/40 transition-all group/it">
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            <span className="text-[10px] font-bold text-zinc-800 uppercase tabular-nums">{lesson.duration}</span>
                                                            {lesson.preview && (
                                                                <span className="text-[9px] font-black bg-[#92E3A9]/10 text-[#92E3A9] px-2 py-0.5 rounded uppercase">Preview</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                ))}
                        ...
                        <section className="p-12 bg-[#92E3A9] rounded-[3.5rem] text-zinc-900">
                             <div className="grid grid-cols-1 md:grid-cols-4 gap-12 items-center">
                                 ...
                                 <div className="md:col-span-3">
                                      <span className="text-[10px] font-black text-black/50 uppercase tracking-[0.3em] mb-2 block">Instruction Lead</span>
                                      ...
                                      <div className="flex flex-wrap gap-6">
                                          <div className="flex flex-col">
                                              <span className="text-[9px] font-black text-black/40 uppercase tracking-widest">Experience</span>
                                              <span className="text-sm font-black uppercase">{course.instructorExp || "10+ Years"}</span>
                                          </div>
                                          <div className="flex flex-col">
                                              <span className="text-[9px] font-black text-black/40 uppercase tracking-widest">Specialization</span>
                                              <span className="text-sm font-black uppercase">High-End Systems</span>
                                          </div>
                                      </div>
                                 </div>
                             </div>
                    ...
                    <div className="space-y-8 h-fit lg:sticky lg:top-32">
                         <div className="bg-[#0A0A0A] border border-zinc-900 rounded-[2.5rem] p-10 shadow-2xl">
                             <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-8">Access Requirements</h4>
                             ...
                             <div className="pt-8 border-t border-zinc-900 flex flex-col gap-4">
                                  <div className="flex items-center justify-between">
                                      <span className="text-[10px] font-black text-zinc-600 uppercase">Valuation</span>
                                      <span className="text-3xl font-black text-[#92E3A9]">{course.price === "0" || !course.price ? "OPEN ACCESS" : `$${course.price}`}</span>
                                  </div>
                                  <button className="w-full bg-[#92E3A9] text-black py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-[#92E3A9]/10">
                                      Initialize Enrollment
                                  </button>
                                  <p className="text-[9px] font-bold text-center text-zinc-700 uppercase leading-relaxed px-4">Instant system access after verification.</p>
                             </div>
                         </div>

                         <div className="bg-zinc-900/30 border border-zinc-900 rounded-[2.5rem] p-10">
                             <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-6">Prerequisites</h4>
                             <ul className="space-y-4">
                                 {course.requirements?.map((req: string, idx: number) => (
                                     <li key={idx} className="flex gap-4 items-center">
                                         <div className="w-1 h-1 bg-zinc-700 rounded-full" />
                                         <span className="text-[11px] font-bold text-zinc-500 uppercase">{req}</span>
                                     </li>
                                 ))}
                             </ul>
                         </div>
...
function SectionHeader({ num, title }: { num: string, title: string }) {
    return (
        <div className="flex items-center gap-6 mb-12">
            ...
            <h2 className="text-xs font-black uppercase tracking-[0.4em] text-zinc-600">{title}</h2>
        </div>
    );
}
